...
const x = new Array(7); //Array constructor function
console.log(x);

// console.log(x.map(() => 5)); doesent work

// x.fill(5);
x.fill(1, 3); //start filling at 3rd index
console.log(x);

arr.fill(23, 4, 6); // 4 to 6 index
console.log(arr);

//Array.from function()

const y = Array.from({ length: 7 }, () => 1); //callig from{a_method} to an Array function.
...
labelBalance.addEventListener('click', function () {
  const movementsUI = Array.from(
    document.querySelectorAll('.movements__value') // Not really an array but an array like structure.But can be easily converted to an array
  );
  console.log(
    movementsUI.map(element => element.textContent.replace('€', ' '))
  );
  const movementsUI2 = [...document.querySelectorAll('.movements__value')]; //Another way using sspred operator
